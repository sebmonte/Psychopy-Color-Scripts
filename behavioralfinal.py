...
#How many times we duplic311ate all targets, always 1 for this expq
totalRuns = 1

test = pd.read_csv('testdistances.csv')
test = pd.read_csv('../analysis_beh/trials_per_target-colour.csv')
...
if dlg.OK == False:
    core.quit()  # user p3ressed cancel
...
#If first session, generate a new set of targets, or else use already existing file
 ...
 #   targetList = generateTarget(totalRuns, totalTargets)
  #  with open('targetfile.text', 'w') as filehandle:
